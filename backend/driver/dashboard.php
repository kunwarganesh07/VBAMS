...
$totalrequest = "SELECT COUNT(*) AS totalrequest FROM tblbook WHERE status='Not Updated Yet' or accepted_by='$id'";
$total_result = $conn->query($totalrequest);
$total_row = $total_result->fetch_assoc();


$cancelrequest = "SELECT COUNT(*) AS cancelrequest FROM tblbook Where status='Rejected' and accepted_by='$id' ";
$cancel_result = $conn->query($cancelrequest);
$cancel_row = $cancel_result->fetch_assoc();


$newrequest = "SELECT COUNT(*) AS newrequest FROM tblbook Where status='Not Updated Yet' and accepted_by='$id' ";
$new_result = $conn->query($newrequest);
$new_row = $new_result->fetch_assoc();

$ontheway = "SELECT COUNT(*) AS ontheway FROM tblbook Where status='On The Way' and accepted_by='$id'";
$ontheway_result = $conn->query($ontheway);
$ontheway_row = $ontheway_result->fetch_assoc();

$complected = "SELECT COUNT(*) AS complected FROM tblbook Where status='Complected'and accepted_by='$id' ";
$complected_result = $conn->query($complected);
$complected_row = $complected_result->fetch_assoc();

$approved = "SELECT COUNT(*) AS approved FROM tblbook Where status!='Approved'and accepted_by='$id' ";
